...
This page records the live creator-workbench shape on Luminal so future
changes to nginx, ``systemd``, config paths, or local TLS can be made from
verified context instead of memory.

Current Host Shape
------------------

Current live creator-workbench deployment:
...
The working pattern for the creator workbench is:

1. mint the leaf certificate for ``creator.pipeworks.luminal.local``
